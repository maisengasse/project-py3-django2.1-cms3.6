...
{% if item[0].publish_workflows %}
        (_(u'Veröffentlichung'), {'fields': ('valid_from', 'valid_to'), 'classes' : ('collapse',)}),
{% else %}
        (None, {'fields': ('published', )}),
{% endif %}
{% if item[0].categorized %}
        (None, {'fields': ({% if item[0].previewpage %}'_link_preview', {% endif %}'category', 'title', 'description', 'images'), }),
{% else %}
        (None, {'fields': ({% if item[0].previewpage %}'_link_preview', {% endif %}'title', 'description', 'images'), }),
{% endif %}
    ]
{% if item[0].previewpage %}
...
{% endif %}
    ...
    _currently_valid.short_description = u"Aktuell öffentlich"
...
{% if item[0].previewpage %}
    def _link_preview(self, obj):
        if obj and obj.id:
            ...
            return u"<em>erst nach dem Speichern möglich</em>"
        return ""
