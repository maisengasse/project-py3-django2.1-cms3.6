...
REMOTE_USER = "julianschwazer"
REMOTE_PATH = "/web/julianschwazer/home/projects/julianschwazer"
REMOTE_DB = "julianschwazerdb2"
REMOTE_MEDIA = [
    REMOTE_PATH + "/media/filer_public/",
    # REMOTE_PATH + "/media/ckeditor/"
]
LOCAL_PATH = "/home/vagrant/julianschwazer"
LOCAL_DB = "julianschwazer"
LOCAL_MEDIA = [
    LOCAL_PATH + "/media/filer_public/",
    # LOCAL_PATH + '/src/julianschwazer/media/ckeditor/'
]
...
INIT_MEDIA_DATA = (
    "rsync -a %(local)s/.ansible/data/filer_public/ %(local)s/media/filer_public/"
)

local = Connection(
    "localhost",
    user="vagrant",
    connect_kwargs=dict(password="vagrant", allow_agent=False, look_for_keys=False),
)
...
@task
def db_from_dump(c, dumpfile="julianschwazer.sync.sql", gzip=False):
    with local.cd(LOCAL_PATH):
...
@task
def get_remote_dump(c, dumpfile="julianschwazer.sync.sql", gzip=False):
    with remote.cd(REMOTE_PATH):
        context = {"db": REMOTE_DB, "dump": dumpfile, "v": REMOTE_MYSQL_VERSION}
        if gzip:
            remote.run(CREATE_GZIP_DUMP % context)
            remote.get(
                "%s/%s.gz" % (REMOTE_PATH, dumpfile),
                "%s/%s.gz" % (LOCAL_PATH, dumpfile),
            )
        else:
            remote.run(CREATE_DUMP % context)
            remote.get(
                "%s/%s" % (REMOTE_PATH, dumpfile), "%s/%s" % (LOCAL_PATH, dumpfile)
            )


@task
def sync_down(c, dumpfile="julianschwazer.sync.sql"):
